  ...
  switch ((cmd ?? '').toLowerCase()) {
    ...
    case '/setblock':
      // ex: /setblock images on, /setblock links off, /setblock mentions on
      {
        ...
        if (what === 'images') MOD_CONFIG.blockImages = on;
        if (what === 'videos') MOD_CONFIG.blockVideos = on;
        if (what === 'audio') MOD_CONFIG.blockAudio = on;
        if (what === 'docs') MOD_CONFIG.blockDocs = on;
        if (what === 'links') MOD_CONFIG.blockLinks = on;
        if (what === 'mentions') MOD_CONFIG.blockMentions = on;
