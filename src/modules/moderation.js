...
export async function moderateMessage(sock, msg) {
    try {
        const key = msg.key;
        ...
        const shouldDelete = (isImage && MOD_CONFIG.blockImages) ||
        ...
        if (shouldDelete) {
            ...
            try {
                ...
                let reason = 'blocked: ';
                if (isImage && MOD_CONFIG.blockImages)
                    reason += 'image ';
                if (isVideo && MOD_CONFIG.blockVideos)
                    reason += 'video ';
                if (isAudio && MOD_CONFIG.blockAudio)
                    reason += 'audio ';
                if (isDoc && MOD_CONFIG.blockDocs)
                    reason += 'document ';
                if (hasLinks && MOD_CONFIG.blockLinks)
                    reason += 'link ';
                if (hasMentions && MOD_CONFIG.blockMentions)
                    reason += 'mention ';
                logModeration(jid, 'delete_message', reason.trim());
                console.log(`🗑️ Message supprimé de ${jid}: ${reason}`);
            }
            catch (e) {
                console.warn('Could not delete message (not admin?)', e?.message || e);
            }
        }
